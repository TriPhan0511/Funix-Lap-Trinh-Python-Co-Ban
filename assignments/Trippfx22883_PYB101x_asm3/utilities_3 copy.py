...
def get_string(msg='Enter: ', err_msg='This part can not be empty.', allow_empty=False):
...
# def get_float(msg='Nhập vào một số: ', err_msg_1='Bạn phải nhập một số.', err_msg_2='Bạn phải nhập một số không âm', allow_empty=False):
#     '''Get a floating-point number from user input.'''
#     while True:
#         inp = get_string(msg, allow_empty=allow_empty)
#         if len(inp) == 0:
...
# # def get_int(msg='Nhập vào một số nguyên: ', err_not_an_integer_msg='Nhập sai. Vui lòng nhập vào một số nguyên.', err_less_than_zero_msg='Bạn phải nhập một số không âm', allow_empty=False):
# def get_int(msg='Enter an integer number: ', err_not_an_integer_msg='You must enter an integer number.', err_less_than_zero_msg='You must enter a number which is greater than or equal to zero.',err_less_than_zero_msg_2='You must enter a number which is greater than zero.', allow_empty=False, err_equal_to_zero__msg='You must enter a number which is greater than zero.', allow_zero=True):
...
#             print(err_equal_to_zero__msg)
#             continue
#         if not allow_zero and int_number == 1:
#             print(err_less_than_zero_msg_2)
#             continue
#         return int_number
...
# def get_days(msg='Nhập số ngày: ', total_days_in_month=31, allow_empty=False, err_msg_1='Nhập sai. Vui lòng nhập vào một số nguyên.', err_msg_2='Có lỗi!'):
#     while True:
#         days = get_int(msg, err_not_an_integer_msg=err_not_an_integer_msg, err_less_than_zero_msg=err_msg_2, allow_empty=allow_empty)
#         # days = get_int(msg, err_msg_1=err_msg, allow_empty=allow_empty)
#         if days == None:
#             return None
#         if days < 0 or days > total_days_in_month:
...
def add_employee(depts, emps):
    ...
    salary_base = get_int(msg='Nhập hệ số lương: ', err_not_an_integer_msg=err_msg_1, err_less_than_zero_msg=err_msg_2)
    working_days = get_days('Nhập số ngày làm việc: ', err_not_an_integer_msg=err_msg_1, err_less_than_zero_msg=err_msg_2)
    working_performance = get_float('Nhập hệ số hiệu quả: ', err_less_than_zero_msg=err_msg_2)
    # todo
...
def edit_employee(emps, msg='Edit Employee'):
    ...
    salary_base = get_int(msg='Nhập hệ số lương: ',
                          err_not_an_integer_msg=err_msg, allow_empty=True)
    if salary_base == None:
