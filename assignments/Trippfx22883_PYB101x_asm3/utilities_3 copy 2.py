...
def get_employee_id(emps, msg='Enter an ID: ', err_msg = 'This ID was taken.\nPlease enter another.'):
    ...
    while True:
        id = get_string(msg=msg, err_empty_msg=ERR_EMPTY_MSG)
        # id = get_string(msg)
        if id.lower() in ids:
            print(err_msg)
            continue
...
def is_manager(msg='Enter a position (NV/QL): ', err_msg='You must enter "QL" if a manager or "NV" if an employee'):
    while True:
        position = get_string(msg=msg, err_empty_msg=ERR_EMPTY_MSG)
        position = position.upper()
        if position not in ['NV', 'QL']:
            print(err_msg)
            continue
...
def get_name(msg='Enter fullname: ', err_empty_msg='This part can not be empty.', allow_empty=False):
    return get_string(msg=msg, err_empty_msg=err_empty_msg, allow_empty=allow_empty)
...
def add_employee(depts, emps):
    ...
    dept_id = get_string(msg='Nhập mã bộ phận: ', err_empty_msg=ERR_EMPTY_MSG)
