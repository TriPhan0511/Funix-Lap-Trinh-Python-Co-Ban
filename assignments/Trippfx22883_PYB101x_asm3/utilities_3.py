...
def get_bonus(msg='Nhập thưởng: ', wrong_format_msg = 'Bạn phải nhập vào một số nguyên.', allow_empty=False):
    while True:
...
def get_salary_base(msg='Nhập hệ số lương: ', wrong_format_msg ='Bạn phải nhập vào một số nguyên', allow_empty=False):
    while True:
        inp = get_string(msg, allow_empty=allow_empty)
        if len(inp) == 0:
            return None
...
def get_days(msg= 'Nhập số ngày: ', wrong_format_msg = 'Bạn phải nhập vào một số nguyên', total_days_in_month=31, allow_empty=False):
    while True:
        inp = get_string(msg=msg, allow_empty=allow_empty)
        if len(inp) == 0:
            return None
...
def get_working_performance(msg ='Nhập hệ số hiệu quả: ', wrong_format_msg='Bạn phải nhập vào một số.', allow_empty=False):
    while True:
        inp = get_string(msg=msg, allow_empty=allow_empty)
...
def add_employee(depts, emps):
    ...
    bonus = get_bonus()
    late_comming_days = get_days(msg='Nhập số ngày đi muộn: ')                                     
...
def edit_employee(emps):
    ...
    wrong_format_msg = 'Bạn cần nhập đúng định dạng'
    ...
    salary_base = get_salary_base(wrong_format_msg=f'{wrong_format_msg}\nBạn phải nhập vào một số nguyên', allow_empty=True)
    if salary_base == None:
        salary_base = emp.salary_base
    working_days = get_days(msg='Nhập số ngày làm việc: ', wrong_format_msg=f'{wrong_format_msg}\nBạn phải nhập vào một số nguyên', allow_empty=True)
    if working_days == None:
        working_days = emp.working_days
    working_performance = get_working_performance(wrong_format_msg=f'{wrong_format_msg}\nBạn phải nhập vào một số', allow_empty=True)
    if working_performance == None:
        working_performance = emp.working_performance
    bonus = get_bonus(wrong_format_msg=f'{wrong_format_msg}\nBạn phải nhập vào một số nguyên', allow_empty=True)
    if bonus == None:
        bonus = emp.bonus
    late_comming_days = get_days(msg='Nhập số ngày đi muộn: ', wrong_format_msg=f'{wrong_format_msg}\nBạn phải nhập vào một số nguyên',  allow_empty=True)
